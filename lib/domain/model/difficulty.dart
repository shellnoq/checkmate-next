...
enum DifficultyLevel {
  ...
  EngineOptions get engineOptions => switch (this) {
        ...
        DifficultyLevel.beginner =>
          const EngineOptions(skillLevel: 0, blunderChance: 0.55, hashMb: 8),
        DifficultyLevel.novice =>
          const EngineOptions(skillLevel: 1, blunderChance: 0.35, hashMb: 8),
        DifficultyLevel.amateur =>
          const EngineOptions(skillLevel: 4, blunderChance: 0.15, hashMb: 16),
        DifficultyLevel.club => const EngineOptions(limitElo: 1500, hashMb: 16),
        DifficultyLevel.candidate =>
          const EngineOptions(limitElo: 1800, hashMb: 32),
        DifficultyLevel.expert =>
          const EngineOptions(limitElo: 2100, hashMb: 32),
        DifficultyLevel.master =>
          const EngineOptions(limitElo: 2400, hashMb: 64),
        DifficultyLevel.grandmaster =>
          const EngineOptions(threads: 2, hashMb: 64),
      };

  /// Kademenin arama sınırları.
  SearchLimits get searchLimits => switch (this) {
        DifficultyLevel.beginner =>
          const SearchLimits(depth: 1, moveTimeMs: 300, multiPv: 5),
        DifficultyLevel.novice =>
          const SearchLimits(depth: 2, moveTimeMs: 300, multiPv: 4),
        DifficultyLevel.amateur =>
          const SearchLimits(depth: 5, moveTimeMs: 400, multiPv: 3),
