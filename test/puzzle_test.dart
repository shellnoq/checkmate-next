...
void main() {
  group('bulmaca verisi', () {
    test('her bulmaca tam olarak bildirilen derinlikte zorunlu mattır', () {
      final ids = <String>{};
  ...
  group('PuzzleSession', () {
    ...
    test('üç hamlelik mat baştan sona oynanabilir', () {
      ...
      while (guard++ < 6) {
        ...
        for (final entry in session.position.legalMoves.entries) {
          for (final to in entry.value.squares) {
            ...
            final after =
                session.position.play(session.position.normalizeMove(move));
...
bool _defenderCannotEscape(Position afterAttacker, int remaining) {
  ...
  for (final entry in afterAttacker.legalMoves.entries) {
    for (final to in entry.value.squares) {
      final next = afterAttacker
          .play(afterAttacker.normalizeMove(NormalMove(from: entry.key, to: to)));
